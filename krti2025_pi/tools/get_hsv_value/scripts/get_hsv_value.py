...
class GetColor:
    ...
    def main(self):
        while True:
            ...
            # print(mask)
            # print(type(mask))
            # Buat copy untuk display tanpa memodifikasi original
            img_display = cv.flip(self.img, 1)
            cv.imshow("img", img_display)
            hsv_display = cv.flip(self.hsv, 1)
            cv.imshow("hsv", hsv_display)
            mask_display = cv.flip(mask, 1)
            cv.imshow("mask", mask_display)
            res_display = cv.flip(res, 1)
            cv.imshow("obj", res_display)
